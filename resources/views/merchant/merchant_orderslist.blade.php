                    ...
                    <div class="card-body pt-0">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                            ...
                            @if (count($orders))
                                ...
                                @php
                                $orderBelongsToMerchant = false;
                                foreach ($order->vproducts as $product) {
                                    if($product->products->first()->merchant_id == $merchantData->id) {
                                        $orderBelongsToMerchant = true;
                                        break;
                                    }
                                }
                                @endphp
                                        ...
                                        <td class="text-end">
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                                                ...
                                                <div class="menu-item px-3">
                                                    <form action="{{route('merchant.orders.view', $order->id)}}" method="GET" id="viewOrder">
                                                        @csrf
                                                    </form>
                                                    <a onclick="document.getElementById('viewOrder').submit();" href="{{route('merchant.orders.view', $order->id)}}" class="menu-link px-3">جزئیات سفارش</a>
                                                </div>
                                                <!--end::Menu item-->
                                                <!--begin::Menu item-->
                                                <div class="menu-item px-3">
                                                    <form action="{{route('merchant.orders.destroy', $order->id)}}" method="POST" id="deleteOrder-{{$order->id}}">
                                                        @csrf
                                                    </form>
                                                    <a onclick="document.getElementById('deleteOrder-{{$order->id}}').submit();" class="menu-link px-3" data-kt-ecommerce-order-filter="delete_row" onclick ="return confirm('آیا برای انجام این کار اطمینان دارید؟')">حذف</a>
                                                </div>
                                                <!--end::Menu item-->
                                            </div>
                                            <!--end::Menu-->
