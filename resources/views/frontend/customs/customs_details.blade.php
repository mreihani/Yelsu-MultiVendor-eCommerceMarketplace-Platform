        ...
        <div class="container">
            <div class="row gutter-lg">
                <aside class="sidebar left-sidebar vendor-sidebar sticky-sidebar-wrapper sidebar-fixed">
                    ...
                    {{-- @php
                        $customsItem_sector_arr = explode(",", $customsItem->vendor_sector);
                        $customsItem_sector_cat_arr = [];
                        foreach ($customsItem_sector_arr as $customsItem_sector_item) {
                            $customsItem_sector_cat_arr[] = App\Models\Category::find($customsItem_sector_item);
                        }
                    @endphp --}}
                    ...
                    <div class="sidebar-content">
                        <div class="sticky-sidebar">
                            <div class="widget widget-collapsible widget-categories">
                                <h3 class="widget-title"><span>زمینه فعالیت منطقه اقتصادی</span></h3>
                                <ul class="widget-body filter-items search-ul">
                                    <a href="">
                                        @if($customsItem->customs_type == "customs")
                                            گمرک
                                        @elseif($customsItem->customs_type == "port")
                                            بندر
                                        @elseif($customsItem->customs_type == "free_zone")
                                            منطقه آزاد
                                        @elseif($customsItem->customs_type == "special_zone")
                                           منطقه ویژه اقتصادی     
                                        @endif
                                    </a>
                                </ul>
                ...
                <div class="main-content">
                    <div class="store store-banner mb-4">
                        <figure class="store-media">
                            <img src="{{(!empty($customsItem->banner_photo)) ? url($customsItem->banner_photo) : url('storage/upload/no_image_store.jpg')}}" alt="Vendor" width="930" height="446"
                                style="background-color: #414960;" />
                        </figure>
                        <div class="store-content">
                            <figure class="seller-brand">
                                <img src="{{(!empty($customsItem->photo)) ? url($customsItem->photo) : url('storage/upload/no_image.jpg')}}" alt="Brand" width="80"
                                    height="80" />
                            </figure>
                            <h4 class="store-title">{{$customsItem->name}}</h4>
                    ...
                    @if($customsItem->about_customs)
                        <h2 class="title vendor-product-title mb-4">
                            <a href="#">اطلاعات منطقه اقتصادی</a>
                        </h2>
                        <div class="product-wrapper row cols-md-12 cols-sm-12 cols-12" style="text-align: justify;">
                            {!! $customsItem->about_customs !!}
                        </div>
                    @endif

                    @if($customsItem->customs_specs)
                        <h2 class="title vendor-product-title mb-4">
                            <a href="#">مشخصات و ویژگی منطقه اقتصادی</a>
                        </h2>
                        <div class="product-wrapper row cols-md-12 cols-sm-12 cols-12" style="text-align: justify;">
                            {!! $customsItem->customs_specs !!}
                        </div>
                    @endif
