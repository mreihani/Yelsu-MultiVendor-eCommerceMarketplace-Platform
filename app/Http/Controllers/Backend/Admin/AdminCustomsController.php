...
class AdminCustomsController extends Controller
{
    public function AdminAllCustomsOutlet()
    {
        $id = Auth::user()->id;
        ...
        return view('admin.customsoutlets.admin_customs_all', compact('adminData', 'customoutlets'));
    }

    public function AdminCreateCustomsOutlet()
    {
        $id = Auth::user()->id;
        $adminData = User::find($id);
        $type = Purify::clean(request('type'));

        return view('admin.customsoutlets.admin_customs_outlet_add', compact('adminData', 'type'));
    }

    public function AdminStoreCustomsOutlet(Request $request)
    {
        ...
        return redirect(route('admin.all.customsoutlet'))->with('success', 'آدرس مورد نظر با موفقیت اضافه گردید.');
    }

    public function AdminEditCustomsOutlet($outlet_id)
    {
        $id = Auth::user()->id;
        $adminData = User::find($id);
        ...
        return view('admin.customsoutlets.admin_customs_edit', compact('adminData', 'outlet'));
    }

    public function AdminUpdateCustomsOutlet(Request $request)
    {
        $outlet_id = Purify::clean(Purify::clean($request->outlet_id));
        ...
        return redirect(route('admin.all.customsoutlet'))->with('success', 'آدرس مورد نظر با موفقیت به روز رسانی گردید');
    }

    public function AdminDeleteCustomsOutlet($outlet_id)
    {
        ...
        return redirect(route('admin.all.customsoutlet'))->with('success', 'آدرس مورد نظر با موفقیت حذف گردید.');
    }
}